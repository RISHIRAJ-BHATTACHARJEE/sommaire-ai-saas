  ...
  return (
    <section className="relative overflow-hidden bg-gray-50">
      <div className="py-12 lg:py-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto relative">
          {steps.map((step, idx) => (
            <div className="relative flex items-stretch" key={idx}>
              <StepItem key={idx} {...step} />
              {idx < steps.length - 1 && (
                <div className="hidden absolute lg:block top-1/2 -right-4 transform -translate-y-1/2 z-10">
                  <MoveRight
                    size={32}
                    strokeWidth={1}
...
function StepItem({ icon, label, description }: Step) {
  return (
    <div className="relative p-10 rounded-2xl bg-white/5 backdrop-blur-xs border-3 border-white/10 hover:border-rose-500/10 transition-colors group w-full mb-6">
      <div className="flex flex-col gap-6 h-full">
        ...
        <div className="flex flex-col flex-1 gap-4 justify-between">
          ...
          <p className="text-center text-gray-600 text-md">{description}</p>
        </div>
      </div>
    </div>
  );
